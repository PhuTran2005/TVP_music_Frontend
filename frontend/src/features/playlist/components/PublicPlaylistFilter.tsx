...
import { Search, ListFilter, Sparkles, XCircle } from "lucide-react";
...
interface PublicPlaylistFilterProps {
...
const SORT_OPTIONS = [
  { label: "Mới cập nhật", value: "newest", icon: Sparkles },
...
export const PublicPlaylistFilter: React.FC<PublicPlaylistFilterProps> = ({
  params,
  onSearch,
  onFilterChange,
  onReset,
  ...
  // Sync từ URL về Input (Khi reset)
  useEffect(() => {
    setLocalSearch(params.keyword || "");
  }, [params.keyword]);

  // Sync từ Input ra URL qua Debounce
  useEffect(() => {
  ...
  // Xác định xem bộ lọc có đang được dùng hay không
  const isFiltering = !!params.keyword;
      ...
      <div className="flex flex-col lg:flex-row gap-4 items-start lg:items-center w-full">
        ...
        <div className="relative w-full lg:max-w-[420px] shrink-0 group">
          ...
          <Input
            ...
            autoComplete="off"
            spellCheck="false"
            // TƯƠNG PHẢN CAO: Nền solid, bóng đổ, viền focus
